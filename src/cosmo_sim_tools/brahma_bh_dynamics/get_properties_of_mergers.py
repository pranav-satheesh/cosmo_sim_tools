...
hubble = 0.6771
from cosmo_sim_tools.arepo_tools.cosmology import *
...
def evaluate(N):
    ...
    for snap in redshift_snapshots_to_search:
        ...
        BH_HostHaloMass = BH_HostHaloMass_dict[snap]
        if (pid in ParticleIDs) & (sid in ParticleIDs):
            found_at_least_one_snap_with_both_ids = 1
            BH_HostHaloMass_primary=BH_HostHaloMass[pid==ParticleIDs]
            BH_HostHaloMass_secondary=BH_HostHaloMass[sid==ParticleIDs]
            if (BH_HostHaloMass_primary!=BH_HostHaloMass_secondary):
                merger_type = 2
                halo_merger_snap_before = snap
                break
            else:
                found_at_least_one_snap_with_both_ids_in_one_halo = 1
                halo_merger_snap_after = snap
                merged_HostHaloMass = BH_HostHaloMass_primary[0]
        else:
    ...
    if(merger_type==0):
        halo_merger_redshift_before = seed_redshift
        halo_merger_redshift_after = merger_redshift   
        merged_HostHaloMass = -1
    elif(merger_type==1):
        halo_merger_redshift_after=z_list[halo_merger_snap_after==snap_list][0]
        halo_merger_redshift_before= seed_redshift
    elif(merger_type==3):
        halo_merger_redshift_after = merger_redshift
        halo_merger_redshift_before= z_list[halo_merger_snap_before==snap_list][0]
        merged_HostHaloMass = -1
    else:
        halo_merger_redshift_after=z_list[halo_merger_snap_after==snap_list][0]
        halo_merger_redshift_before=z_list[halo_merger_snap_before==snap_list][0]

    mask_trajectory = (Redshift_details >= merger_redshift-redshift_tolerance) & (Redshift_details <= halo_merger_redshift_before+redshift_tolerance)
    ...
    merger_time_scale_upper=T(merger_redshift,halo_merger_redshift_before)/hubble/1e6
    merger_time_scale_lower=T(merger_redshift,halo_merger_redshift_after)/hubble/1e6
    ...
    merger_properties['halo_merger_redshift_after'].append(halo_merger_redshift_after)
    merger_properties['halo_merger_redshift_before'].append(halo_merger_redshift_before)
    merger_properties['merged_HostHaloMass'].append(merged_HostHaloMass)
...
#print(z_list)
if (os.path.exists(basePath+'blackhole_details.hdf5')==False):
...
BH_HostHaloMass_dict = dict.fromkeys(snap_list)

for snap,desired_redshift in list(zip(snap_list,z_list)):    
    ParticleIDs,o=arepo_package.get_particle_property(basePath,'ParticleIDs',5,desired_redshift)
    BH_HostHaloMass,o=arepo_package.get_particle_property(basePath,'BH_HostHaloMass',5,desired_redshift)
    ParticleID_dict[snap]=ParticleIDs
    BH_HostHaloMass_dict[snap]=BH_HostHaloMass
...
keys = ['merger_redshift','seed_redshift','merger_time_scale_best','merger_time_scale_err','halo_merger_redshift_after','halo_merger_redshift_before','merged_HostHaloMass','merger_type']
...
numpy.save(basePath + '/merger_statistics.npy',[merger_properties,initial_trajectory_primary,initial_trajectory_secondary])
