...
def nfw_profile_final(r, rho_s, r_s):
    """NFW density profile function"""
    return rho_s / ((r/r_s) * (1 + r/r_s)**2)

def fit_nfw_profile(r, rho, p0=None):
    """
    ...
    # Perform the fit
    popt, _ = curve_fit(nfw_profile, r, rho, p0=p0,method='trf',maxfev=100000)
...
rho_s_final_space,r_s_final_space=[],[]
#f,ax = plt.subplots(1,1,figsize=(10,10))
ii=0
for profile,bin_centers in list(zip(DM_density_profiles_space,bin_centers_space)):
    mask=(profile>0) & (bin_centers < numpy.log10(Group_R_Mean200_space[ii])) 
    ...
    if (len(profile[mask])==0):
        ...
        r_s_final_space.append(-1)       
        continue
    
    r_s_space = numpy.logspace(-2,2,100)
    print(ii,r,rho)    
    residuals_pre = 1000
    for r_s_guess in r_s_space:
        rho_s, r_s = fit_nfw_profile(r, rho,p0=[0.000001,r_s_guess])
        #print(r_s,r_s_guess)
        rho_fit = nfw_profile(10**(bin_centers[mask]),rho_s,r_s)        
        residuals = sum((numpy.log10(profile[mask]) - numpy.log10(rho_fit))**2)
        if (residuals < residuals_pre):
            residuals_pre = residuals
            rho_s_final, r_s_final = rho_s,r_s
        #print(residuals_pre,rho_s_final,r_s_final)

    print(f"Best-fit parameters: rho_s = {rho_s_final:.3e}, r_s = {r_s_final:.3f}")
    
    rho_s_final_space.append(rho_s_final)
    r_s_final_space.append(r_s_final)
...
r_s_final_space = numpy.array(r_s_final_space)

numpy.save(output_dir+'/NFW_fit_parameters.npy',[rho_s_final_space,r_s_final_space])
