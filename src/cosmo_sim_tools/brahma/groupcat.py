...
def gcPath3(basePath, snapNum, chunkNum=0):
    """ Return absolute path to a group catalog HDF5 file (modify as needed). """
    gcPath = basePath + '/groups_%03d/' % snapNum
    filePath1 = gcPath + 'groups_%03d.%d.hdf5' % (snapNum, chunkNum)
    filePath2 = gcPath + 'fof_tab_%03d.%d.hdf5' % (snapNum, chunkNum)

    if isfile(filePath1):
        return filePath1
    return filePath2

def gcPath4(basePath, snapNum, chunkNum=0):
    """ Return absolute path to a group catalog HDF5 file (modify as needed). """
    gcPath = basePath + '/groups_sub_%03d/' % snapNum
    filePath1 = gcPath + 'groups_sub_%03d.%d.hdf5' % (snapNum, chunkNum)
    filePath2 = gcPath + 'fof_sub_tab_%03d.%d.hdf5' % (snapNum, chunkNum)

    if isfile(filePath1):
        return filePath1
    return filePath2

def gcPath_postprocessed(basePath, snapNum, chunkNum=0):
    """ Return absolute path to a group catalog HDF5 file (modify as needed). """
    gcPath = basePath + '/groups_%03d_postprocessed/' % snapNum
    filePath1 = gcPath + 'groups_%03d_postprocessed.%d.hdf5' % (snapNum, chunkNum)
    filePath2 = gcPath + 'fof_subhalo_tab_%03d_postprocessed.%d.hdf5' % (snapNum, chunkNum)
...
def loadObjects_postprocessed(basePath, snapNum, gName, nName, fields):
    ...
    with h5py.File(gcPath_postprocessed(basePath, snapNum), 'r') as f:
    ...
    for i in range(header['NumFiles']):
        f = h5py.File(gcPath_postprocessed(basePath, snapNum, i), 'r')
...
def loadObjects2(basePath, snapNum, gName, nName, fields):
    ...
    for i in range(header['NumFiles']):
        f = h5py.File(gcPath2(basePath, snapNum, i), 'r')
...
def loadObjects3(basePath, snapNum, gName, nName, fields):
    """ Load either halo or subhalo information from the group catalog. """
    result = {}
    ...
    # load header from first chunk
    with h5py.File(gcPath3(basePath, snapNum), 'r') as f:
    ...
    for i in range(header['NumFiles']):
        f = h5py.File(gcPath3(basePath, snapNum, i), 'r')
...
def loadObjects4(basePath, snapNum, gName, nName, fields):
    """ Load either halo or subhalo information from the group catalog. """
    result = {}
    ...
    # load header from first chunk
    with h5py.File(gcPath4(basePath, snapNum), 'r') as f:
    ...
    for i in range(header['NumFiles']):
        f = h5py.File(gcPath4(basePath, snapNum, i), 'r')
...
def loadSubhalos_postprocessed(basePath, snapNum, fields=None):
    """ Load all subhalo information from the entire group catalog for one snapshot
       (optionally restrict to a subset given by fields). """

    return loadObjects_postprocessed(basePath, snapNum, "Subhalo", "subgroups", fields)
...
def loadHalos_postprocessed(basePath, snapNum, fields=None):
    """ Load all halo information from the entire group catalog for one snapshot
       (optionally restrict to a subset given by fields). """

    return loadObjects_postprocessed(basePath, snapNum, "Group", "groups", fields)
...
def loadHalos3(basePath, snapNum, fields=None):
    """ Load all halo information from the entire group catalog for one snapshot
       (optionally restrict to a subset given by fields). """

    return loadObjects3(basePath, snapNum, "Group", "groups", fields)

def loadHalos4(basePath, snapNum, fields=None):
    """ Load all halo information from the entire group catalog for one snapshot
       (optionally restrict to a subset given by fields). """

    return loadObjects4(basePath, snapNum, "Group", "groups", fields)
